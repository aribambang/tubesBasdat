...
<table id="dg" class="easyui-datagrid" style="width:80%;height:410px"
    url="http://localhost/tubes1/modul/matkul/get_users.php"
    title="Laboran"
    toolbar="#toolbar"
	  fitColumns="true"
	  singleSelect="true"
    ...
    <thead>
        <tr>
            <th field="kd_matkul" style="width:20%; background-color:#E0EDFF">Kode Mata Kuliah</th>
            <th field="nama_matkul" width="30%">Nama Mata Kuliah</th>
            <th field="prodi_matkul" width="20%">Program Studi</th>
            <th field="dosen_pengampu" width="20%">Dosen Pengampu</th>
	...
	<div id="dlg" class="easyui-dialog" style="width:500px;height:280px;padding:10px 20px"
        ...
        <tr>
          ...
          <td>:</td>
          <td><input name="kd_matkul" id="kd_matkul" class="easyui-textbox" required="true" style="width:200px;"></td>
        </tr>
				<tr>
					<td>Nama Mata Kuliah</td>
					<td>:</td>
					<td><input name="nama_matkul" id="nama_matkul" class="easyui-textbox" required="true" style="width:200px;"></td>
				</tr>
        <tr>
					<td>Program Studi</td>
					<td>:</td>
					<td><input name="prodi_matkul" id="prodi_matkul" class="easyui-textbox" required="true" style="width:200px;"></td>
				</tr>
        <tr>
					<td>Dosen Pengampu</td>
					<td>:</td>
					<td><input name="dosen_pengampu" id="dosen_pengampu" class="easyui-textbox" required="true" style="width:200px;"></td>
				</tr>
        <tr>
					<td>SKS</td>
					<td>:</td>
					<td><input name="sks" id="sks" class="easyui-numberbox" required="true" style="width:200px;"></td>
				</tr>
				</table>
...
//			document.getElementById("update").value="Insert Data";
			...
			$('#dlg').dialog('open').dialog('setTitle','Informasi Mata Kuliah');
			$('#fm').form('clear');
			url = 'http://localhost/tubes1/modul/matkul/save_users.php';
		}
		function editUser(){
			var str = "Update Data";
			document.getElementById('update').value = str;
			...
			if (row){
				$('#dlg').dialog('open').dialog('setTitle','Informasi Mata Kuliah');
				$('#fm').form('load',row);
				url = 'http://localhost/tubes1/modul/matkul/update_users.php';
			}
			else if (row==null){
		...
		function destroyUser(){
			...
			if (row){
				...
				$.messager.confirm('Konfirmasi','Apakah anda yakin menghapus data ini?',function(r){
					if (r){
						$.post('http://localhost/tubes1/modul/matkul/delete_users.php',{kd_matkul:row.kd_matkul,tombol:str},function(result){
							if (result.success){
								$('#dg').datagrid('reload');
