...
<table id="dg" class="easyui-datagrid" style="width:80%;height:410px"
    url="http://localhost/tubes1/modul/ruangan/get_users.php"
    title="Laboran"
    toolbar="#toolbar"
	  fitColumns="true"
	  singleSelect="true"
    ...
    <thead>
        <tr>
            <th field="kd_ruangan" style="width:20%; background-color:#E0EDFF">Kode Ruangan</th>
            <th field="nama_ruangan" width="80%">Nama Ruangan</th>
        </tr>
    </thead>
</table>
	<div id="toolbar">
	...
	<div id="dlg" class="easyui-dialog" style="width:500px;height:280px;padding:10px 20px"
        ...
        <tr>
          <td>Kode Ruangan</td>
          <td>:</td>
          <td><input name="kd_ruangan" id="kd_ruangan" class="easyui-textbox" required="true" style="width:200px;"></td>
        </tr>
				<tr>
					<td>Nama Ruangan</td>
					<td>:</td>
					<td><input name="nama_ruangan" id="nama_ruangan" class="easyui-textbox" required="true" style="width:200px;"></td>
				</tr>
				</table>
...
//			document.getElementById("update").value="Insert Data";
			...
			$('#dlg').dialog('open').dialog('setTitle','Informasi Ruangan');
			$('#fm').form('clear');
			url = 'http://localhost/tubes1/modul/ruangan/save_users.php';
		}
		function editUser(){
			var str = "Update Data";
			document.getElementById('update').value = str;
			...
			if (row){
				$('#dlg').dialog('open').dialog('setTitle','Informasi Ruangan');
				$('#fm').form('load',row);
				url = 'http://localhost/tubes1/modul/ruangan/update_users.php';
			}
			else if (row==null){
		...
		function destroyUser(){
			...
			if (row){
				...
				$.messager.confirm('Konfirmasi','Apakah anda yakin menghapus data ini?',function(r){
					if (r){
						$.post('http://localhost/tubes1/modul/ruangan/delete_users.php',{kd_ruangan:row.kd_ruangan,tombol:str},function(result){
							if (result.success){
								$('#dg').datagrid('reload');
